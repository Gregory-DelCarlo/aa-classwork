...
def casablanca_cast
  ...
  join
    ...
    -- map the castings table onto the actors table where any actor_id is 
    -- in the same row as the equivalent actors.id 
  join
    movies on castings.movie_id = movies.id
    -- map the movies table onto the castings table where any movies.id is
    -- in the same row as the equivalent movie_id
  where
